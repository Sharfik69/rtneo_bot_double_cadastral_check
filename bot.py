...
def load_file(message):
    try:
        ...
        bot.send_message(message.chat.id,
                         'Файл добавлен, теперь мне нужно название страницы' + file_name)
...
def main(message, file_name, sheet_name, num):
    ...
    try:
        ...
        os.remove(file_name)
    if not status:
        bot.send_message(message.chat.id, 'В шагах выше вы указали неприавльную информацию, скорее всего неверное имя страницы')
        os.remove(file_name)
        return
    else:
